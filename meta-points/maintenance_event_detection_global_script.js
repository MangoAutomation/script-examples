...
//to store watchListId's
var maintenanceEventXid = new ArrayList();
var MaintenanceEventsInfo = new HashMap();
...
function getWatchListMaintenanceEventsInfo(){
    var maintenanceEventDao = Java.type('com.serotonin.m2m2.maintenanceEvents.MaintenanceEventDao');
    ...
    var allMaintenanceEvents = maintenanceEventDaoInstance.getAll();
    var watchListMaintenanceEventsCount = 0;
    var totalWatchListMaintenanceEventsCount = 0;

    var maintenanceEventsInfo = new HashMap();
    var testInfo = new HashMap();

    for each (var maintenanceEvent in allMaintenanceEvents){

        var objMaintenanceEventModel = new maintenanceEventModel(maintenanceEvent);

        if(objMaintenanceEventModel.getMaintenanceType() == "WATCHLIST"){

             var astNodeInitial = new ASTNode("eq", "typeName", 'MAINTENANCE');
             var rql = RQLUtils.addAndRestriction(astNodeInitial, new ASTNode("eq", "rtnTs", null));
             rql = RQLUtils.addAndRestriction(rql, new ASTNode("eq", "rtnApplicable", "Y"));
             rql = RQLUtils.addAndRestriction(rql, new ASTNode("in", "typeRef1", objMaintenanceEventModel.id));

             watchListMaintenanceEventsCount = eventInstanceServiceClass.customizedCount(rql);
             if(watchListMaintenanceEventsCount > 0)
             {
               maintenanceEventXid.add(objMaintenanceEventModel.getWatchListXid());
               maintenanceEventsInfo.put("watchListXid",objMaintenanceEventModel.getWatchListXid());
             }
         }
    }
      return maintenanceEventsInfo;
}

function getAllWatchListsActiveMaintenanceEventsDataPoints(){
    var watchListService = Java.type('com.infiniteautomation.mango.spring.service.WatchListService');
    var watchListServiceClass = Common.getBean(watchListService.class);
    var watchListVo = Java.type('com.serotonin.m2m2.watchlist.WatchListVO');
    //loop through global watchListXidsList
     for each (var xid in maintenanceEventXid){
         var watchListVO = watchListServiceClass.get(xid);
         //grab the watchListId here
         var watchListId = watchListVO.id;
         var watchListXid = watchListVO.xid;

         var watchListDataPointsConsumer = Java.extend(Consumer, {
             accept:function(dpId){
               var result = new HashMap();
               result.put("dataPointId", dpId.id);
               result.put("dataPointXid", dpId.xid);
               result.put("watchListId", watchListId);
               result.put("watchListXid", watchListXid);
               dpList.add(result);
          }
        });
         ...
         var  watchListType = watchListVO.getType();
          ...
          if(watchListType == "STATIC") {
             var wid = watchListVO.id;
             watchListServiceClass.getDataPoints(xid,new watchListDataPointsConsumer());
           }
         else if(watchListType == "TAGS"){
             LOG.debug("Currently Mango doesn't support TAGS WatchList type...",watchListVO.getType);
         }
         else if(watchListType == "QUERY"){
            LOG.debug("Currently Mango doesn't support QUERY WatchList type...",watchListVO.getType);
         }
      }
    return dpList;
}
